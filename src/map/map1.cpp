...
void Map1::initBlocks(SDL_Renderer *renderer)
{
    ...
    // Block decoration (Stuff)
    BlockVec.push_back(new Block(20, 3, 4, 1, 64, true));
    BlockVec[BlockVec.size() - 1]->blockEngine(renderer, grass_decor,
        {{1, 0, 2, 1}}
    );
    BlockVec.push_back(new Block(24, 4, 4, 1, 64, true));
    BlockVec[BlockVec.size() - 1]->blockEngine(renderer, grass_decor,
        {{0, 2, 1, 0}}
    );
    BlockVec.push_back(new Block(28, 5, 4, 1, 64, true));
    BlockVec[BlockVec.size() - 1]->blockEngine(renderer, grass_decor,
        {{2, 1, 1, 2}}
    );
}
...
{
    DecorFrontVec.push_back(new Decoration("res/Decoration/Tree1.png", 42, 7, 4.3, 5));
...
void Map1::updateMapExclusive(SDL_Renderer *renderer, Player *player, Input *input, Collision *collision)
{
    ...
    for (float i = 0; i < BackgroundVec.size(); i+=2)
    {
        ...
        if (i > 0)
        {
            int backgroundPos = player->getY() - Game::HEIGHT / 2;
            backgroundPos = backgroundPos < 0 ? 0 : backgroundPos;
            BackgroundVec[i]->setY((backgroundPos * i/200) + Game::HEIGHT / 2);
            BackgroundVec[i+1]->setY((backgroundPos * i/200) + Game::HEIGHT / 2);
        }
    }
}